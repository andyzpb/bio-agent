...
function App(): React.ReactElement {
  ...
  const [chatApprovalBusy, setChatApprovalBusy] = useState<Record<string, boolean>>({});
    ...
    while (true) {
      ...
      for (const chunk of chunks) {
        ...
        if (parsed.event === "connected") {
          updateStoredChatSeq(sessionKey, parsed.data, { authoritative: true });
  ...
  const resolveChatApproval = useCallback(async (approvalId: string, action: "approve" | "reject") => {
    const cleanApprovalId = approvalId.trim();
    if (!cleanApprovalId || chatApprovalBusy[cleanApprovalId]) return;
    setChatApprovalBusy((current) => ({ ...current, [cleanApprovalId]: true }));
    try {
      await api(`/api/dashboard/chat/approvals/${encodePath(cleanApprovalId)}/${action}`, {
        method: "POST",
      });
      appendChatEvent({
        event: action === "approve" ? "approval_resolved" : "approval_rejected",
        kind: action === "approve" ? "approval_resolved" : "approval_rejected",
        label: action === "approve" ? "Approval approved" : "Approval rejected",
      ...
      setChatApprovalBusy((current) => {
        const next = { ...current };
        delete next[cleanApprovalId];
        return next;
      });
    }
  }, [appendChatEvent, chatApprovalBusy, chatSessionKey]);
      ...
      <main className={`workspace${isPluginWorkbench ? " plugin-workbench-mode" : ""}`}>
        ...
        {isChatView ? (
          <ChatPane
            ...
            approvalBusy={chatApprovalBusy}
            onResolveApproval={(approvalId, action) => void run(async () => {
              await resolveChatApproval(approvalId, action);
            })}
            onOpenSession={() => {
        ...
        <>
          <div className="chat-stream" ref={streamRef}>
            {props.events.length ? props.events.map((event) => <ChatEventItem
              key={event.id}
              event={event}
              approvalBusy={event.approval_id ? !!props.approvalBusy[event.approval_id] : false}
              onResolveApproval={props.onResolveApproval}
            />) : (
              <div className="chat-empty">
                <div className="chat-empty-kicker">Biomedical Evidence Agent</div>
                <div className="chat-empty-title">Ask for a traceable evidence workflow.</div>
                ...
                <div className="chat-empty-prompts">
                  {[
                    "Summarize recent evidence on microglial activation in Alzheimer's disease.",
                    "Build an evidence packet for SGLT2 inhibitors and heart failure outcomes.",
                    "Audit this claim against retrieved citations and flag overclaims.",
                  ].map((prompt) => (
          ...
          <form className="chat-composer" onSubmit={(event) => { event.preventDefault(); props.onSend(); }}>
            <textarea
              value={props.input}
              placeholder="Ask for evidence, audit, provenance, or a research-only workflow..."
...
function ChatEventItem(props: {
  ...
  if (event.event === "approval_required") {
    ...
    const status = event.status || "approval_required";
    const isPending = status === "approval_required" || status === "pending";
    return (
      <div className={`chat-event chat-approval-card ${isPending ? "pending" : "resolved"}`}>
        <div className="chat-approval-head">
          <span className="status-pill">{status}</span>
          <span>{event.tool_name || event.label}</span>
          {event.risk && <small>{event.risk}</small>}
        </div>
        {event.reason && <div className="chat-approval-reason">{event.reason}</div>}
        <div className="chat-approval-grid">
          <div>
            <span>arguments</span>
            <JsonTreeBlock data={event.arguments ?? {}} />
          </div>
          <div>
            <span>final arguments</span>
            <JsonTreeBlock data={event.final_arguments ?? event.arguments ?? {}} />
          </div>
        </div>
        {approvalId && isPending ? (
          <div className="chat-approval-actions">
            <button className="primary" type="button" disabled={props.approvalBusy} onClick={() => props.onResolveApproval(approvalId, "approve")}>
              {props.approvalBusy ? "处理中" : "Approve"}
            </button>
            <button className="danger-ghost" type="button" disabled={props.approvalBusy} onClick={() => props.onResolveApproval(approvalId, "reject")}>
              Reject
            </button>
          </div>
        ) : approvalId ? (
          <div className="chat-approval-actions muted-text">{status}</div>
        ) : null}
      </div>
    );
  }
...
function mergeChatEvent(current: ChatEventRow[], next: ChatEventRow): ChatEventRow[] {
  ...
  if ((next.event === "approval_resolved" || next.event === "approval_rejected") && next.approval_id) {
    const actualIndex = current.findIndex((item) => (
      item.session_key === next.session_key
      && item.event === "approval_required"
      && item.approval_id === next.approval_id
    ));
    if (actualIndex >= 0) {
      const target = current[actualIndex];
      const merged: ChatEventRow = {
        ...target,
        status: next.status ?? (next.event === "approval_resolved" ? "approved" : "rejected"),
        detail: next.detail ?? target.detail,
        reason: next.reason ?? target.reason,
        ts: next.ts ?? target.ts,
      };
      return [...current.slice(0, actualIndex), merged, ...current.slice(actualIndex + 1)];
    }
...
function handleChatSsePayload(
  ...
  if (event === "approval_required") {
    ctx.appendChatEvent({
      event,
      kind: "tool",
      label: String(data.label ?? data.tool_name ?? "Approval required"),
      detail: String(
        data.detail ?? data.reason ?? data.tool_name ?? "Approval required",
      ),
      session_key: String(data.session_key ?? ""),
      tool_name: typeof data.tool_name === "string" ? data.tool_name : undefined,
      call_id: typeof data.call_id === "string" ? data.call_id : undefined,
      ...
      arguments: data.arguments,
      final_arguments: data.final_arguments,
      status: typeof data.status === "string" ? data.status : "approval_required",
      iteration: typeof data.iteration === "number" ? data.iteration : undefined,
      source: "live",
    });
    ctx.setChatLiveEvent(String(data.label ?? data.tool_name ?? "Approval required"));
    return;
  }
  if (event === "approval_resolved" || event === "approval_rejected") {
    ctx.appendChatEvent({
      event,
      kind: event,
      label: String(data.label ?? (event === "approval_resolved" ? "Approval approved" : "Approval rejected")),
      detail: String(data.detail ?? data.reason ?? ""),
      session_key: String(data.session_key ?? ""),
      tool_name: typeof data.tool_name === "string" ? data.tool_name : undefined,
      call_id: typeof data.call_id === "string" ? data.call_id : undefined,
      approval_id: typeof data.approval_id === "string" ? data.approval_id : undefined,
      risk: typeof data.risk === "string" ? data.risk : undefined,
      reason: typeof data.reason === "string" ? data.reason : undefined,
      status: typeof data.status === "string" ? data.status : event === "approval_resolved" ? "approved" : "rejected",
      source: "live",
    });
    ctx.setChatLiveEvent(String(data.label ?? ""));
    return;
  }
  if (event === "step") {
    ctx.appendChatEvent({
  ...
  ctx.appendChatEvent({
    event,
    kind: String(data.kind ?? (event === "approval_required" ? "tool" : "system")),
