...
class DashboardChatMultiplexer:
    ...
    def start(self) -> None:
        ...
        if self._event_bus is not None:
            ...
            self._event_bus.on(ToolApprovalResolved, self._on_approval_resolved)
    ...
    async def _on_tool_completed(self, event: ToolCallCompleted) -> None:
        ...
        if event.status == "approval_required":
            reason = event.reason or event.result_preview
            await self._publish(
                event.session_key,
                "approval_required",
                {
                    ...
                    "risk": event.risk,
                    "reason": _redact_dashboard_chat_string(reason),
                    "detail": _preview_text(
                        _redact_dashboard_chat_string(reason),
                        360,
                    ),
                    "status": event.status,
                    "arguments": _sanitize_dashboard_chat_value(dict(event.arguments)),
    ...
    async def _on_approval_resolved(self, event: ToolApprovalResolved) -> None:
        ...
        await self._publish(
            event.session_key,
            "approval_resolved" if event.status == "approved" else "approval_rejected",
            {
                "session_key": event.session_key,
                "kind": (
                    "approval_resolved"
                ...
                "label": f"Approval {event.status}: {event.tool_name}",
                "tool_name": event.tool_name,
                "call_id": event.call_id,
                "approval_id": event.approval_id,
                "status": event.status,
                "risk": event.risk,
                "reason": _redact_dashboard_chat_string(event.reason),
            },
        )
    ...
    async def _publish(
        ...
        async with self._lock:
            ...
            if event in {"done", "error"}:
                ...
                self._history_by_session.pop(session_key, None)
        if not queues:
            return
        for queue in queues:
            try:
...
def create_dashboard_app(
    ...
    *,
    ...
    app.state.tool_approval_broker = tool_approval_broker
    ...
    @app.get("/api/dashboard/chat/approvals")
    async def list_chat_approvals(
        session_key: str = _DASHBOARD_CHAT_DEFAULT_SESSION,
    ) -> dict[str, Any]:
        clean_session_key = _validate_dashboard_chat_session(session_key)
        if tool_approval_broker is None:
            return {
                "enabled": False,
                "items": [],
                "session_key": clean_session_key,
            }
        approvals = await tool_approval_broker.list_pending(
            session_key=clean_session_key,
        )
        return {
            "enabled": True,
            "items": [
                _sanitize_dashboard_chat_value(item.public_payload())
                for item in approvals
            ],
            "session_key": clean_session_key,
        }

    async def _approve_chat_tool(approval_id: str) -> dict[str, Any]:
        if tool_approval_broker is None:
            raise HTTPException(status_code=503, detail="approval broker 未启用")
        approval_id = str(approval_id or "").strip()
        if not approval_id:
            raise HTTPException(status_code=400, detail="approval_id 不能为空")
        result = await tool_approval_broker.approve(approval_id)
        if result.status == "missing":
            raise HTTPException(status_code=404, detail="approval 不存在或已失效")
        if result.status != "approved":
            raise HTTPException(
                status_code=409,
                detail=result.message or f"approval already {result.status}",
            )
        return {
            "approval_id": approval_id,
            "status": result.status,
            "item": (
                _sanitize_dashboard_chat_value(result.approval.public_payload())
                if result.approval is not None
                else None
            ),
        }

    async def _reject_chat_tool(approval_id: str) -> dict[str, Any]:
        if tool_approval_broker is None:
            raise HTTPException(status_code=503, detail="approval broker 未启用")
        approval_id = str(approval_id or "").strip()
        if not approval_id:
            raise HTTPException(status_code=400, detail="approval_id 不能为空")
        result = await tool_approval_broker.reject(approval_id)
        if result.status == "missing":
            raise HTTPException(status_code=404, detail="approval 不存在或已失效")
    ...
    @app.post("/api/dashboard/chat/approvals/{approval_id}/approve")
    async def approve_chat_tool_by_path(approval_id: str) -> dict[str, Any]:
        return await _approve_chat_tool(approval_id)

    @app.post("/api/dashboard/chat/approvals/{approval_id}/reject")
    async def reject_chat_tool_by_path(approval_id: str) -> dict[str, Any]:
        return await _reject_chat_tool(approval_id)

    @app.post("/api/dashboard/chat/approve")
    async def approve_chat_tool(payload: ChatApprovalPayload) -> dict[str, Any]:
        return await _approve_chat_tool(payload.approval_id)

    @app.post("/api/dashboard/chat/reject")
    async def reject_chat_tool(payload: ChatApprovalPayload) -> dict[str, Any]:
        return await _reject_chat_tool(payload.approval_id)
