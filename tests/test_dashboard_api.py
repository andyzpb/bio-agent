...
@pytest.mark.asyncio
async def test_dashboard_chat_multiplexer_clears_replay_after_done() -> None:
    mux = DashboardChatMultiplexer(bus=None, event_bus=None)
    await mux.publish_user_message_accepted(
        session_key="dashboard:default",
        content="first",
...
async def test_dashboard_chat_multiplexer_maps_lifecycle_events_and_redacts() -> None:
    ...
    try:
        ...
        await mux._on_tool_completed(
            ToolCallCompleted(
                ...
                result_preview="needs secret=abc123456 confirmation",
                approval_id="approval-123",
                risk="writes_storage",
                reason="needs token=abc123456 confirmation",
            )
        )
        approval_event, approval_payload = await asyncio.wait_for(
            queue.get(),
            timeout=1.0,
        )
        await mux._on_approval_resolved(
            ToolApprovalResolved(
                session_key="dashboard:default",
                channel="dashboard",
                chat_id="default",
                ...
                status="approved",
                risk="writes_storage",
                reason="approved token=abc123456",
            )
        )
        resolved_event, resolved_payload = await asyncio.wait_for(
            queue.get(),
            timeout=1.0,
        )
    finally:
    ...
    assert delta_payload["kind"] == "assistant_delta"
    ...
    assert approval_event == "approval_required"
    assert approval_payload["approval_id"] == "approval-123"
    assert approval_payload["risk"] == "writes_storage"
    assert approval_payload["arguments"]["api_key"] == "[redacted]"
    assert approval_payload["final_arguments"]["token"] == "[redacted]"
    assert "[redacted]" in approval_payload["reason"]
    assert resolved_event == "approval_resolved"
    assert resolved_payload["approval_id"] == "approval-123"
    assert resolved_payload["status"] == "approved"
    assert "[redacted]" in resolved_payload["reason"]


def test_dashboard_chat_approval_endpoints_list_and_resolve(tmp_path) -> None:
    ...
    with TestClient(
        create_dashboard_app(tmp_path, tool_approval_broker=broker)
    ) as client:
        list_resp = client.get(
            "/api/dashboard/chat/approvals",
            params={"session_key": "dashboard:default"},
        )
        approve_resp = client.post(
            "/api/dashboard/chat/approvals/approval-api/approve",
        )
        missing_resp = client.post(
            "/api/dashboard/chat/approvals/approval-api/reject",
        )

    assert list_resp.status_code == 200
    payload = list_resp.json()
    assert payload["enabled"] is True
    assert payload["items"][0]["approval_id"] == "approval-api"
    assert payload["items"][0]["arguments"]["api_key"] == "[redacted]"
    assert payload["items"][0]["final_arguments"]["token"] == "[redacted]"
    assert "[redacted]" in payload["items"][0]["reason"]
    assert approve_resp.status_code == 200
    assert approve_resp.json()["status"] == "approved"
    assert missing_resp.status_code == 409


def test_dashboard_chat_approval_legacy_payload_route_still_works(tmp_path) -> None:
    broker = ToolApprovalBroker(ttl_seconds=5)
    ...
    async def _submit() -> None:
        await broker.submit(
            ...
            final_arguments={"project_id": "project-1"},
